...
class StudentFeePaymentController extends Controller
{
    ...
    public function billdeskPayment($uuid, $record_id)
    {
        $this->authorize('makePaymentAsParent', StudentRecord::class);

        $student_record = $this->student_record->findByUuidOrFail($uuid, $record_id);

        return $this->repo->billdeskPayment($student_record, $this->request->all());
    }

    public function billdeskStatus()
    {
        $data = $this->repo->billdeskStatus();

        $status           = gv($data, 'status');
        $message          = gv($data, 'message');
        $reference_number = gv($data, 'reference_number');
        $payment          = gv($data, 'payment');

        if (! $status) {
            return view('errors.billdesk', compact('message', 'payment', 'reference_number'));
        }
        
        return redirect('/student/' . $payment->student_uuid . '/fee/' . $payment->student_record_id);
    }
}